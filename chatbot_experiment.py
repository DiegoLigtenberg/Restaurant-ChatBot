...
class chatbot:
    ...
    def __init__(self, classifier, filename, name_experiment, exp_num, ask_reqs):
        ...
        self.path = os.path.join(os.getcwd(), self.name_experiment)
        self.number_of_inputs = 0
        self.sentences = []
        self.first_area = None
        self.first_food = None
        self.first_pricerange = None
        self.exp_num = exp_num
        self.did_not_understand = 0
        if not os.path.exists(self.path):
            os.makedirs(self.path)
        if check_file(self.path, self.exp_num):
            print("You already did this experiment. We will now close the program")
            sys.exit()

    def first_food_found(self):
        if self.first_food == None:
            self.first_food = self.number_of_inputs

    def first_area_found(self):
        if self.first_area == None:
            self.first_area = self.number_of_inputs

    def first_pricerange_found(self):
        if self.first_pricerange == -1:
            self.first_pricerange = self.number_of_inputs

    def add_did_not_understand(self):
        self.did_not_understand += 1
    ...
    def handle_input(self):
        ...
        self.sentences.append(userInput.split(" "))
    ...
    def create_dataset(self):
        len_sentences = np.array([len(listElem)
                                  for listElem in self.sentences])
        total_sen = np.sum(len_sentences)
        median_sen = np.median(len_sentences)
        average_sen = np.mean(len_sentences)
        std_sen = np.std(len_sentences)
        d = {"experiment": [self.exp_num], "number of inputs": [self.number_of_inputs], "food found": [self.first_food], "area found": [self.first_area],
             "pricerange found": [self.first_pricerange], "total words": [total_sen], "average of sentences": [average_sen],
             "median of sentences": [median_sen], "sd of sentences": [std_sen], "did_not_understand": [self.did_not_understand]}
        df = pd.DataFrame(data=d)
        df.to_csv(os.path.join(self.path, self.exp_num + ".csv"))
        f = open(os.path.join(self.path, self.exp_num + ".txt"), "+w")
        for s in chat:
            f.write(f"{s}\n")
        f.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--reqs", default=False, type=bool,
                        help="Will turn on or of to ask for more requirements.")
    ...
    chatter = chatbot(classifier, "restaurant_info.csv",
                      args.name, args.experiment, args.reqs)
    chatter.start()
